...
public class DodgeGui {
	...
	public void onRenderDodgeGUIEvent(RenderGameOverlayEvent.Post event) {
		if (event.getType() == ElementType.FOOD && ConfigHandler.hud) {
			...
			renderFeathers(event.getWindow().getScaledHeight(),
					...
					ClientStorage.healing, 16, 25, 34, 43, 52, 61, 70, PatronRewardHandler.getTier(Minecraft.getInstance().player), event);
